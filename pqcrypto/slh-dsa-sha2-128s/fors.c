 ...
 *
 * xCoin: FIPS 205 (SLH-DSA) bit order. Upstream PQClean implements SPHINCS+
 * v3.1, which reads the index bits least-significant-bit first within each
 * byte; the base_2^b function of FIPS 205 reads them most-significant-bit
 * first. For the sha2-128s-simple parameter set this is the only observable
 * difference between the two (key generation is identical). Verified against
 * OpenSSL 3.6.1 and the NIST ACVP SLH-DSA-SHA2-128s sigVer vectors; see the
 * file PQCLEAN in this directory.
 */
static void message_to_indices(uint32_t *indices, const unsigned char *m) {
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        ...
        for (j = 0; j < SPX_FORS_HEIGHT; j++) {
            indices[i] ^= (uint32_t)(((m[offset >> 3] >> (7 - (offset & 0x7))) & 0x1) << (SPX_FORS_HEIGHT - 1 - j));
            offset++;
        }
    }
}
