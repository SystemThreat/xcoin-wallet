...
int PQCLEAN_MLDSA65_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    /* H-03: never proceed with key generation on CSPRNG failure — a silent
     * failure could return a key derived from uninitialised/weak entropy. */
    if (randombytes(seedbuf, SEEDBYTES) != 0) {
        PQCLEAN_cleanse(seedbuf, sizeof(seedbuf));
        return -1;
    }
    ...
    /* xCoin: wipe the secret intermediates (xi, rhoprime, K, s1, s2, t0)
     * with a cleanse the optimizer cannot remove. */
    PQCLEAN_cleanse(seedbuf, sizeof(seedbuf));
    PQCLEAN_cleanse(&s1, sizeof(s1));
    PQCLEAN_cleanse(&s1hat, sizeof(s1hat));
    PQCLEAN_cleanse(&s2, sizeof(s2));
    PQCLEAN_cleanse(&t0, sizeof(t0));

    return 0;
}

/*************************************************
* Name:        PQCLEAN_MLDSA65_CLEAN_crypto_sign_keypair_from_seed
*
* Description: Deterministic keypair generation. Identical to
*              crypto_sign_keypair but derives (rho, rhoprime, key) from a
*              caller-supplied 32-byte seed (the FIPS 204 ML-DSA.KeyGen_internal
*              xi input) instead of drawing fresh randomness. Feeding the same
*              seed always yields the same (pk, sk) — this is what makes
*              BIP-39/SHAKE-256 HD derivation of ML-DSA keys recoverable.
*
* Arguments:   - uint8_t *pk:         output public key (1952 bytes)
*              - uint8_t *sk:         output secret key (4032 bytes)
*              - const uint8_t *seed: 32-byte deterministic seed (xi)
*
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLDSA65_CLEAN_crypto_sign_keypair_from_seed(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
    ...
    /* Use the caller-provided seed as xi (no randombytes) */
...
/*************************************************
* Name:        PQCLEAN_MLDSA65_CLEAN_crypto_sign_sk_to_pk
*
* Description: Recompute the public key from a secret key. The ML-DSA secret
*              key stores (rho, K, tr, t0, s1, s2); the public key is
*              (rho, t1) where t1 = HighBits(A*s1 + s2). This recomputes t1
*              from s1/s2/rho exactly as keygen does, so a secret key loaded
*              from disk without its pubkey can still recover it.
*
* Arguments:   - uint8_t *pk:       output public key (1952 bytes)
*              - const uint8_t *sk: input secret key (4032 bytes)
*
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLDSA65_CLEAN_crypto_sign_sk_to_pk(uint8_t *pk, const uint8_t *sk) {
    uint8_t rho[SEEDBYTES];
    uint8_t tr[TRBYTES];
    uint8_t key[SEEDBYTES];
    polyvecl mat[K];
    polyvecl s1, s1hat;
    polyveck s2, t1, t0, t0dummy;
    ...
    PQCLEAN_MLDSA65_CLEAN_polyveck_add(&t1, &t1, &s2);
    PQCLEAN_MLDSA65_CLEAN_polyveck_caddq(&t1);
    PQCLEAN_MLDSA65_CLEAN_polyveck_power2round(&t1, &t0dummy, &t1);
    PQCLEAN_MLDSA65_CLEAN_pack_pk(pk, rho, &t1);

    /* xCoin: wipe the unpacked secret key components with a cleanse the
     * optimizer cannot remove. */
    PQCLEAN_cleanse(key, sizeof(key));
    PQCLEAN_cleanse(&s1, sizeof(s1));
    PQCLEAN_cleanse(&s1hat, sizeof(s1hat));
    PQCLEAN_cleanse(&s2, sizeof(s2));
    PQCLEAN_cleanse(&t0, sizeof(t0));
    PQCLEAN_cleanse(&t0dummy, sizeof(t0dummy));
...
int PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(uint8_t *sig,
    ...
    /* H-03: never proceed with signing on CSPRNG failure. seedbuf holds
     * secret key material (rho, key), so wipe it before failing. */
...
rej:
    ...
    *siglen = PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES;
    ...
    /* xCoin: wipe the secret intermediates (K, rnd, rhoprime, s1, s2, t0,
     * the masks y and the rejected z and w0 candidates) with a cleanse the
     * optimizer cannot remove. */
    PQCLEAN_cleanse(seedbuf, sizeof(seedbuf));
    PQCLEAN_cleanse(&s1, sizeof(s1));
    PQCLEAN_cleanse(&s2, sizeof(s2));
    PQCLEAN_cleanse(&t0, sizeof(t0));
    PQCLEAN_cleanse(&y, sizeof(y));
    PQCLEAN_cleanse(&z, sizeof(z));
    PQCLEAN_cleanse(&w0, sizeof(w0));
    return 0;
}
