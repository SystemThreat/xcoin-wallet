...
/* xCoin local change (see ../slh-dsa-sha2-128s/PQCLEAN): the hash state lives
 * inline in the context instead of behind a malloc'd pointer, so the *_init
 * and *_ctx_clone functions cannot fail (upstream calls exit(111) when malloc
 * returns NULL, which is unacceptable inside consensus signature verification)
 * and *_ctx_release wipes the state instead of freeing it. The API and every
 * hash output are unchanged. */
